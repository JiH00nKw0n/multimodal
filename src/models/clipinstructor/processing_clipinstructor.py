...
__all__ = ["CLIPInstructorProcessor"]

IMG_PROCESSOR_CLASS = "CLIPImageProcessor"
TOKENIZER_CLASS = "T5TokenizerFast"


class CLIPInstructorProcessorKwargs(ProcessingKwargs, total=False):
...
@registry.register_processor("CLIPInstructorProcessor")
class CLIPInstructorProcessor(ProcessorMixin):
    ...
    def from_text_vision_pretrained(
        ...
        tokenizer = T5TokenizerFast.from_pretrained(text_pretrained_model_name_or_path)
    ...
    def __call__(
            ...
            audio=None,
            videos=None,
            **kwargs: Unpack[CLIPInstructorProcessorKwargs],
    ) -> BatchEncoding:

        if text is None and images is None:
            raise ValueError("You must specify either text or images.")
        output_kwargs = self._merge_kwargs(
            CLIPInstructorProcessorKwargs,
